...
/**
 *	Controller de Roster Mensual para desplegar y permitir cambios
 *  XALFEIRAN 2016
 */
class MonthlyRoster extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Monthlyroster_model');
	...
	public function index()
	{
		...
		$selectedmonth = $this->input->get('inputSelectedMonth');
		
		if($selectedmonth != '')
		{
			$fechaini = $this->input->post('fechaini') ? $this->input->post('fechaini') : date('Y-m-d',strtotime("{$selectedmonth}"));
			$fechafin = $this->input->post('fechafin') ? $this->input->post('fechafin') : date('Y-m-d',strtotime("last day of {$selectedmonth}"));
		}
		else{
			$fechaini = $this->input->post('fechaini') ? $this->input->post('fechaini') : date('Y-m-d',strtotime("first day of this month"));
			$fechafin = $this->input->post('fechafin') ? $this->input->post('fechafin') : date('Y-m-d',strtotime("last day of this month"));	
		...
		$posiciones = $this->Posiciones_model->CompanyPositions($idempresa);

		$agents = $this->Agentes_model->StationAgents($idempresa,$idoficina);

		//$rows = $this->MonthlyRoster_model->LoadRosterArrayFechas($idempresa,$idoficina,$fechaini, $fechafin);
		...
		//$data['rows'] = $rows;
		$data['fechaini'] = strtotime($fechaini);
		$data['fechafin'] = strtotime($fechafin);
		$data['selectedmonth'] = $selectedmonth;
		$data['posiciones'] = $posiciones;
		$data['fullagents'] =  $agents;
		...
		if($data)
		{
			...
			$this->load->view('New2Monthlyroster_view',$data);
	...
	public function AsyncLoadMonthlyRoster(){
		// data de la sesion
		$idempresa = $this->session->userdata('idempresa');
		$idoficina = $this->session->userdata('idoficina');
		...
		$fechaini = date('Y-m-d',strtotime("{$selectedmonth}"));
		$fechafin = date('Y-m-d',strtotime("last day of {$selectedmonth}"));
